...
from config import OUTPUT_DIR, create_logger

# --- evaluate Config ---
evaluate = create_logger("evaluate", log_to_file=True)
...
def explain_with_shap(model, X, model_name: str, suffix: str = "", show: bool = False) -> pd.DataFrame:
    ...
    """
    evaluate.info(f"🔍 Running SHAP for {model_name} ({suffix})...")
    try:
        ...
        evaluate.info(f"✅ SHAP explainability complete. Top feature: {df.iloc[0]['feature']}")
        return df

    except Exception as e:
        evaluate.warning(f"⚠️ SHAP failed: {e}. Falling back to permutation importance.")
        try:
            ...
            evaluate.info("✅ Fallback importance computed.")
            return df

        except Exception as e2:
            evaluate.error(f"❌ Fallback also failed: {e2}")
...
def plot_heatmap(df_importance: pd.DataFrame, model_name: str, suffix: str = ""):
    ...
    """
    evaluate.info("📌 Generating strategy combo heatmap...")
    try:
        ...
        plt.clf()
        evaluate.info(f"✅ Heatmap saved: {out_path}")
    except Exception as e:
        evaluate.error(f"❌ Heatmap generation failed: {e}")
